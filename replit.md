...
- **iOS Safe Area & Scroll Architecture**: `body { position: fixed; overflow: hidden }` and `#root { position: absolute; inset: 0; overflow-y: auto }` prevent WKWebView bounce. All sticky page headers use `paddingTop: env(safe-area-inset-top)`. Sub-pages (approvals, rewards-board, settings, skins-gallery, achievements, my-rewards, my-achievements, family-goals) use `calc(Xrem + env(safe-area-inset-top))` on their root container. A global rAF loop in App.tsx enforces `window.scrollY = 0` at 60fps to prevent WKWebView UIScrollView drift from keyboard/camera/system events. Radix `DialogContent` prevents auto-focus by default (`onOpenAutoFocus: e.preventDefault()`) to stop iOS from scrolling when dialogs open. Both dashboards freeze their scroll container during any dialog open and restore the position with a 350ms delayed second restore after close.
