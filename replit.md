...
- **Shared Tasks**: New task type where multiple specific children complete a task together and split points equally. Uses `isSharedTask` (boolean) and `sharedMemberIds` (text array) fields. Points calculated as `Math.floor(totalPoints / numberOfMembers)`. Mutually exclusive with multi-completion tasks. Task cards show member avatars with completion status (e.g., "1/2 done"). Each child can only complete once (backend prevents duplicate submissions). Full i18n support (8 languages).
