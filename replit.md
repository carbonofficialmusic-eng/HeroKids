...
- **Multi-Completion Tasks**: Tasks can have a `maxCompletions` value allowing multiple family members to complete the same task. The system displays a counter (e.g., "2/3") showing progress. For recurring multi-completion tasks, the task remains visible when fully completed (grayed out until reset at midnight). For non-recurring multi-completion tasks, the task disappears after reaching maxCompletions. The backend uses SQL atomic increments with FOR UPDATE locks to prevent race conditions. `nextAvailableDate` is only set when maxCompletions is reached for recurring tasks, keeping the task available for other children until fully completed.
