...
- **Authentication**: First-party email/password auth using bcrypt password hashes, Passport sessions, PostgreSQL session storage, email verification, password reset tokens, login rate limiting, case-insensitive email lookup, and mobile JWT/device-token compatibility. Verified account emails are protected by a normalized unique database index. Legacy `/api/login` and `/api/callback` redirect to `/`.
