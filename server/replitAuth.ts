    ...
    app.get(
      ...
      (req: any, res, next) => {
        passport.authenticate("google", (err: any, user: any, info: any) => {
          ...
          // ⚠️  Passport 0.6+ calls req.session.regenerate() inside req.logIn,
          // which wipes every custom key we stored before the OAuth redirect.
          // Capture the values NOW, before logIn destroys the old session.
          ...
          req.logIn(user, (loginErr: any) => {
            ...
            // (googleNative / googleNativePollKey were on the OLD session which
            // was already regenerated by req.logIn — nothing to delete here)
            req.session.save((saveErr: any) => {
