  ...
  const handleAppleSignIn = async () => {
    ...
    try {
      ...
      if (result.response.identityToken && data.user) {
        ...
        setFormMessage(t("landing.auth.loginFailed", "Anmeldung fehlgeschlagen."));
      }
    } catch (e: any) {
      if (e?.message?.includes("SIGN_IN_CANCELLED") || e?.code === "1001") {
        // user cancelled — no error message
      } else {
        setFormMessage(e.message || t("landing.auth.loginFailed", "Apple-Anmeldung fehlgeschlagen."));
      }
    } finally {
            ...
            <>
              <button
                ...
                style={{
                  ...
                  color: "#fff", fontFamily: "'Nunito', sans-serif", fontWeight: 600, fontSize: "0.9rem",
                }}
              >
                ...
                {t("landing.auth.appleButton", "Continue with Apple")}
              </button>
