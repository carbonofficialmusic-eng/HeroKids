...
import { Plus, Trophy, Gift, Star, Crown, BarChart3, Settings, Trash2, Pencil, Lightbulb, Check, X, MessageCircle, ClipboardCheck, Target, Share2, Users, CheckCircle2, Clock, Coins } from "lucide-react";
import { apiRequest, queryClient } from "@/lib/queryClient";
import { useToast } from "@/hooks/use-toast";
import { Link } from "wouter";
import type { FamilyMember, Task, Reward, RewardRequest, RewardRedemption } from "@shared/schema";
...
export default function Dashboard() {
  ...
  // Fetch my reward redemptions (for showing sharing options)
  const { data: myRedemptions = [] } = useQuery<(RewardRedemption & { rewardTitle?: string })[]>({
    queryKey: ["/api/reward-redemptions"],
    enabled: !!member,
  });

  // Fetch shared rewards
        ...
        {isParent ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6 min-w-0">
              ...
              {(() => {
                // Get own redemptions with sharing status
                const ownRedemptions = myRedemptions.filter(r => r.redeemedBy === member?.id && r.status === "approved");
                // Merge with shared rewards data for participant info
                const redemptionsWithDetails = ownRedemptions.map(r => {
                  const sharedInfo = sharedRewards.find(s => s.id === r.id);
                  return {
                    ...r,
                    sharingStatus: sharedInfo?.sharingStatus || r.sharingStatus || "not_shared",
                    participants: sharedInfo?.participants || [],
                    originalPointsSpent: sharedInfo?.originalPointsSpent || r.pointsSpent,
                  };
                });

                if (redemptionsWithDetails.length === 0) return null;

                return (
                  <div>
                    <h2 className="text-2xl font-bold font-accent mb-4">{t("kidDashboard.myRewards")}</h2>
                    <div className="grid md:grid-cols-2 gap-4">
                      {redemptionsWithDetails.map((redemption) => {
                        const isSharing = redemption.sharingStatus === "sharing_active";
                        const isFinalized = redemption.sharingStatus === "sharing_finalized";
                        const participants = redemption.participants || [];
                        const hasParticipants = participants.length > 0;
                        const totalParticipants = participants.length + 1; // Include initiator

                        return (
                          <Card key={redemption.id} className="p-4" data-testid={`card-my-redemption-${redemption.id}`}>
                            <div className="flex items-start gap-3">
                              <div className="h-10 w-10 rounded-full gradient-winner flex items-center justify-center shrink-0">
                                <Gift className="h-5 w-5 text-white" />
                              </div>
                              <div className="flex-1 min-w-0">
                                <h3 className="font-bold text-sm mb-1">{redemption.rewardTitle || t("kidDashboard.reward")}</h3>
                                <div className="flex items-center gap-2 mb-2">
                                  <Badge variant={isFinalized ? "default" : isSharing ? "secondary" : "outline"} className="text-xs">
                                    <Coins className="h-3 w-3 mr-1" />
                                    {isFinalized 
                                      ? `${Math.floor(redemption.originalPointsSpent / totalParticipants)} pts (${t("kidDashboard.shared")})`
                                  ...
                                  {isSharing && (
                                    <Badge variant="secondary" className="text-xs">
                                      ...
                                      {t("kidDashboard.sharingActive")}
                                    </Badge>
                                  )}
                                  {isFinalized && (
                                    <Badge variant="default" className="text-xs">
                                      <CheckCircle2 className="h-3 w-3 mr-1" />
                                      {t("kidDashboard.finalized")}
                                    </Badge>
                                  )}
                                </div>

                                {/* Participants display */}
                                {(isSharing || isFinalized) && (
                                  <div className="mb-2">
                                    <p className="text-xs text-muted-foreground mb-1">
                                      <Users className="h-3 w-3 inline mr-1" />
                                      {hasParticipants 
                                        ? `${totalParticipants} ${t("kidDashboard.participants")}`
                                        : t("kidDashboard.noParticipantsYet")
                                      }
                                    </p>
                                    {hasParticipants && (
                                      <div className="flex -space-x-2">
                                        {participants.slice(0, 5).map((p: any) => (
                                          <Avatar key={p.id} className="h-6 w-6 border-2 border-background">
                                            ...
                                            <AvatarFallback style={{ backgroundColor: p.member?.color }} className="text-white text-xs">
                                ...
                                {/* Action buttons */}
                                <div className="flex gap-2 flex-wrap">
                                  {/* Start Sharing - only if not shared yet */}
                                  {redemption.sharingStatus === "not_shared" && (
                                    <Button
                                      ...
                                      data-testid={`button-start-sharing-${redemption.id}`}
                                    >
                                      <Share2 className="h-3 w-3 mr-1" />
                                      {t("kidDashboard.shareReward")}
                                    </Button>
                                  )}

                                  {/* Cancel Sharing - only if sharing is active with no participants */}
                                  {isSharing && !hasParticipants && (
                                    <Button
                                      size="sm"
                                      variant="outline"
                                  ...
                                  {/* Finalize Sharing - only if sharing is active with participants */}
                                  {isSharing && hasParticipants && (
                                    <Button
                                      size="sm"
                                      onClick={() => finalizeSharingMutation.mutate(redemption.id)}
                                      disabled={finalizeSharingMutation.isPending}
                                      data-testid={`button-finalize-sharing-${redemption.id}`}
                                    >
                                      <CheckCircle2 className="h-3 w-3 mr-1" />
                                      {t("kidDashboard.finalizeSharing")}
                                    </Button>
                                  )}
                                </div>
                              </div>
