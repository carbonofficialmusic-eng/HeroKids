          ...
          <div ref={containerRef} className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative">
            ...
            <div className="lg:col-span-2 space-y-6 min-w-0">
              ...
              {isRealParent && rewardRequests.filter(r => r.status === "pending").length > 0 && (
                <Card className="p-6 bg-primary/5 border-primary/20 hover-elevate overflow-visible mb-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      <MessageCircle className="h-6 w-6 text-primary" />
                      <h3 className="text-xl font-bold font-accent">{t("dashboard.pendingRewardRequests")}</h3>
                    </div>
                  ...
                  <div className="space-y-4">
                    {rewardRequests
                      ...
                      .map((request) => {
                        ...
                        return (
                          <div key={request.id} className="flex items-center justify-between p-3 bg-card rounded-xl border border-border/50">
                            <div className="flex items-center gap-3">
                              <Avatar className="h-8 w-8">
                                <AvatarImage src={getAvatarUrl(requester?.activeSkinId, requester?.avatarUrl, (requester as any)?.useCustomAvatar, (requester as any)?.updatedAt)} />
                                <AvatarFallback style={{ backgroundColor: requester?.color }} className="text-white text-xs">
                                  {requester?.displayName[0]}
                                </AvatarFallback>
                              </Avatar>
                              <div>
                                <p className="font-bold text-sm leading-none">{request.title}</p>
                                <p className="text-xs text-muted-foreground mt-1">
                                  {requester?.displayName} • {request.pointThreshold} pts
                                </p>
                              </div>
                            </div>
                    ...
                    {rewardRequests.filter(r => r.status === "pending").length > 3 && (
                      <p className="text-center text-xs text-muted-foreground pt-1">
                        {t("dashboard.andMoreRequests", { count: rewardRequests.filter(r => r.status === "pending").length - 3 })}
                      </p>
                    )}
                  </div>
                </Card>
