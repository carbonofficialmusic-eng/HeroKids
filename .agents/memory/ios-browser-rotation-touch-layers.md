...
**How to apply:** Treat this as a stale WebKit route subtree rather than an ordinary overlay or CSS pointer-events bug. Do not gate the repair on User-Agent strings or touch-capability values because Replit's embedded browser may report them unexpectedly. Listen to explicit orientation signals plus layout and visual-viewport resize signals, but only infer rotation from resize when viewport width changes substantially. Keyboard opening changes height and must never remount the page. Wait for rotation to settle, then remount only the affected interactive page subtree. Keep authentication, global providers, query cache, and background wrapper outside the remount boundary.